...
use crate::web2::proto::http2::ping;
...
enum Kind {
    ...
    Chan {
        ...
        data_rx: mpsc::Receiver<Result<Bytes, crate::web2::error::Error>>,
...
impl Body {
    ...
    fn poll_eof(&mut self, cx: &mut task::Context<'_>) -> Poll<Option<crate::web2::error::Result<Bytes>>> {
    ...
    fn poll_inner(&mut self, cx: &mut task::Context<'_>) -> Poll<Option<crate::web2::error::Result<Bytes>>> {
        match self.kind {
...
impl HttpBody for Body {
    type Data = Bytes;
    type Error = crate::web2::error::Error;
...
impl Sender {
    ...
    pub fn poll_ready(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::web2::error::Result<()>> {
        // Check if the receiver end has tried polling for the body yet
        ready!(self.poll_want(cx)?);
        self.data_tx
            .poll_ready(cx)
            .map_err(|_| crate::web2::error::Error::new_closed())
    }

    fn poll_want(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::web2::error::Result<()>> {
        match self.want_rx.load(cx) {
            WANT_READY => Poll::Ready(Ok(())),
            WANT_PENDING => Poll::Pending,
            watch::CLOSED => Poll::Ready(Err(crate::web2::error::Error::new_closed())),
            unexpected => unreachable!("want_rx value: {}", unexpected),
        }
    }

    async fn ready(&mut self) -> crate::web2::error::Result<()> {
        futures_util::future::poll_fn(|cx| self.poll_ready(cx)).await
    }

    /// Send data on data channel when it is ready.
    pub async fn send_data(&mut self, chunk: Bytes) -> crate::web2::error::Result<()> {
        self.ready().await?;
        self.data_tx
            .try_send(Ok(chunk))
            .map_err(|_| crate::web2::error::Error::new_closed())
    ...
    pub async fn send_trailers(&mut self, trailers: HeaderMap) -> crate::web2::error::Result<()> {
        let tx = match self.trailers_tx.take() {
            Some(tx) => tx,
            None => return Err(crate::web2::error::Error::new_closed()),
        };
        tx.send(trailers).map_err(|_| crate::web2::error::Error::new_closed())
    }
    ...
    pub fn abort(self) {
        let _ = self
            ...
            .clone()
            .try_send(Err(crate::web2::error::Error::new_body_write_aborted()));
    }
